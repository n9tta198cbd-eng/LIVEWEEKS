...
const translations = {
    ...
    ru: {
        ...
        automationInstruction: 'Откройте приложение Команды → Автоматизация → Новая → Время суток → 6:00 → Повторять "Ежедневно" → "Немедленный запуск" → "Создать новую команду (новая автоматизация)"',
        shortcutInstruction: 'ДОБАВЬТЕ ЭТИ ДЕЙСТВИЯ:',
        action1Text: '3.1 "Получить содержимое URL" → вставьте URL:',
        action2Text: '3.2 "Установить фото как обои" → "Экран блокировки"',
        importantNote: '*ВАЖНО: В "Установить фото обоев" нажмите стрелочку в кружке → отключите "Обрезать до темы" и "Показать окно просмотра".'
    }
};
